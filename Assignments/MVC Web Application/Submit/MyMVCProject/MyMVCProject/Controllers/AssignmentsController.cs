    ...
    public class AssignmentsController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "AssignmentID,EventID,VolunteerID")] Assignment assignment)
        {
            ...
            else
            {
                ...
                if (overlapped != null)
                {
                    error = String.Format("This assignment conflicts with this event:{0}, start:{1}, end:{2} ", overlapped.Description, overlapped.StartDate, overlapped.EndDate);
                }
            }
        ...
        {
            var a1 = db.Assignments.Find(assignment.AssignmentID);
            if (a1.EventID == assignment.EventID && a1.VolunteerID == assignment.VolunteerID)
            {
                return RedirectToAction("Index");
            }

            string error = null;
            var found = db.Assignments.Where(a => a.EventID == assignment.EventID && a.VolunteerID == assignment.VolunteerID);
            if (found.Any())
            {
                error = String.Format("Volunteer {0} is already assigned for another event.", a1.Volunteer.LastName);
            }
            else
            {
                var e = db.Events.Find(assignment.EventID);
                ...
                foreach (Assignment a in v.Assignments)
                {
                    if (a.Event.overlappedWith(e) && a.AssignmentID != assignment.AssignmentID)
                    {
                        overlapped = a.Event;
                    }
                }
                ...
                {
                    a1.EventID = assignment.EventID;
                    a1.VolunteerID = assignment.VolunteerID;
                    db.SaveChanges();
                    return RedirectToAction("Index");
                }
            }
